...
interface ChildStatistics {
  ...
  postureCompletionCounts: Record<string, number>;
...
interface SessionData {
  ...
  dateTime: string;
  totalTime: number;
  completedPostures: number;
  accuracy: number;
  sessionNumber: number;
  postureTimes?: Record<string, number | null>;
}
...
export default function MirrorPostureGameInsights() {
  ...
  const [gameHistory, setGameHistory] = useState<MirrorPostureGameRecord[]>([]);
  ...
  const loadStatistics = async (childId: string) => {
    ...
    try {
      const response = await fetch(`http://localhost:8083/api/mirror-posture-game/child/${childId}/statistics`);
      ...
      if (response.ok) {
        ...
        console.log('Session Data:', sessions);
           ...
           const processedSessions = sessions.map((session: any) => {
             const totalTime = (session.lookingLeft || 0) + (session.lookingRight || 0) + 
                              (session.mouthOpen || 0) + (session.showingTeeth || 0) + (session.kiss || 0);
             
             const completedPostures = [session.lookingLeft, session.lookingRight, session.mouthOpen, 
                                      session.showingTeeth, session.kiss].filter(time => time && time > 0).length;
             
             const accuracy = completedPostures > 0 ? (completedPostures / 5) * 100 : 0;
             
             return {
               sessionId: session.sessionId,
               dateTime: session.dateTime,
               totalTime: totalTime,
               completedPostures: completedPostures,
               accuracy: accuracy,
  ...
  const loadGameHistory = async (childId: string, page: number = 0) => {
    try {
      const response = await fetch(`http://localhost:8083/api/mirror-posture-game/child/${childId}/history?page=${page}&size=5`);
  ...
  const getAverageTime = (postureName: string): number => {
    if (!statistics?.averageCompletionTimes) return 0;
    return statistics.averageCompletionTimes[postureName] || 0;
  };

  const getCompletionCount = (postureName: string): number => {
    if (!statistics?.postureCompletionCounts) return 0;
    return statistics.postureCompletionCounts[postureName] || 0;
  };

  const getCompletionRatio = (postureName: string): number => {
    const totalGames = statistics?.totalGames || 0;
    if (totalGames === 0) return 0;
    return (getCompletionCount(postureName) / totalGames) * 100;
  };
  ...
  const getPerformanceMetrics = () => {
    ...
    const averageSessionTime = sessionData.reduce((sum, session) => sum + session.totalTime, 0) / totalSessions;
    const bestSession = sessionData.reduce((best, current) => current.totalTime < best.totalTime ? current : best);
    const worstSession = sessionData.reduce((worst, current) => current.totalTime > worst.totalTime ? current : worst);
    const averageAccuracy = sessionData.reduce((sum, session) => sum + session.accuracy, 0) / totalSessions;
    
    // Calculate improvement trend
    const recentSessions = sessionData.slice(-5);
    const improvementTrend = recentSessions.length > 1 ? 
      ((recentSessions[0].totalTime - recentSessions[recentSessions.length - 1].totalTime) / recentSessions[0].totalTime) * 100 : 0;
  ...
  // Prepare pie chart data for posture distribution
  const getPostureDistributionData = () => {
    return POSTURE_NAMES.map((name, index) => ({
      name: name.split(' ')[0], // Just the first word for display
      value: getCompletionCount(name),
      color: COLORS[index % COLORS.length]
    })).filter(item => item.value > 0);
  };

  // Calculate consistency data for radar chart
  const getConsistencyData = () => {
    return POSTURE_NAMES.map((name, index) => {
      // Calculate consistency score based on completion ratio and time variance
      const completionRatio = getCompletionRatio(name);
      const averageTime = getAverageTime(name);
      
      // Simple consistency score: higher completion ratio = higher consistency
      // In a real implementation, you might want to calculate variance of completion times
      const consistencyScore = completionRatio > 0 ? Math.min(100, completionRatio + (averageTime > 0 ? 20 : 0)) : 0;
  ...
  // Prepare chart data
  const averageTimesData = POSTURE_NAMES.map((name, index) => ({
    posture: name,
  ...
  const postureDistributionData = getPostureDistributionData();
        ...
        <div className="text-center py-12">
          <div className="mb-8">
            <h1 className="text-5xl font-playful text-transparent bg-clip-text bg-gradient-to-r from-blue-600 via-purple-600 to-pink-600 mb-4">
              Mirror Posture Game Hub 🎭
            </h1>
            <p className="text-xl font-comic text-gray-600 max-w-2xl mx-auto">
              Master facial expressions with fun challenges! Play new games and track your progress.
            </p>
          </div>
        </div>
          ...
          <Card className="card-playful backdrop-blur-sm bg-white/90 border-2 border-yellow-200 shadow-xl">
            <CardContent className="p-12 text-center">
              ...
              <p className="text-lg font-comic text-muted-foreground mb-8 max-w-md mx-auto">
                {selectedChild.name} hasn't played the Mirror Posture Game yet. 
                Start playing to unlock amazing insights and track your progress!
              </p>
              <Button 
                onClick={() => navigate('/games/mirror-posture')}
          ...
          <Card className="card-playful backdrop-blur-sm bg-gradient-to-br from-blue-50 to-purple-50 border-2 border-blue-200 shadow-2xl max-w-md mx-auto">
            <CardContent className="p-4 text-center">
              ...
              <p className="text-xs font-comic text-gray-600 mb-3 max-w-sm mx-auto">
                Keep improving your facial expression skills!
              </p>
              <Button 
                onClick={() => navigate('/games/mirror-posture')}
                   ...
                   <div className="relative bg-gradient-to-br from-white/95 to-white/85 backdrop-blur-sm rounded-3xl border-2 border-blue-200/50 shadow-2xl p-8">
                     <div className="text-center mb-8">
                       ...
                       <p className="text-lg font-comic text-gray-600">
                         Let's see how {selectedChild.name} is doing with facial expressions!
                       </p>
                     </div>
                       ...
                       <div className="group relative">
                         ...
                         <div className="relative bg-gradient-to-br from-blue-50 to-indigo-50 rounded-2xl border-2 border-blue-200 p-6 text-center hover:scale-105 transition-all duration-300 shadow-lg hover:shadow-2xl">
                           ...
                           <div className="text-4xl font-bold text-blue-600 mb-2">
                             {performanceMetrics?.averageSessionTime ? performanceMetrics.averageSessionTime.toFixed(1) : '0'}s
                           </div>
                       ...
                       <div className="group relative">
                         ...
                         <div className="relative bg-gradient-to-br from-purple-50 to-pink-50 rounded-2xl border-2 border-purple-200 p-6 text-center hover:scale-105 transition-all duration-300 shadow-lg hover:shadow-2xl">
                           ...
                           <div className="text-4xl font-bold text-purple-600 mb-2">
                             {performanceMetrics?.averageAccuracy ? performanceMetrics.averageAccuracy.toFixed(0) : '0'}%
                           </div>
                           ...
                           {performanceMetrics?.bestSession ? (
                             <>
                               <div className="text-3xl font-bold text-emerald-600 mb-2">
                                 {performanceMetrics.bestSession.totalTime.toFixed(1)}s
                               </div>
                               <div className="text-sm font-comic text-emerald-700 font-semibold">
                                 Best Session
                               </div>
                               <div className="text-xs text-emerald-600 mt-1">
                                 #{performanceMetrics.bestSession.sessionNumber}
                               </div>
                             </>
                           ) : (
                             <>
                     ...
                     <div className="relative bg-gradient-to-br from-green-50/90 to-emerald-50/90 backdrop-blur-sm rounded-3xl border-2 border-green-200/70 shadow-xl hover:shadow-2xl transition-all duration-300 overflow-hidden">
                       ...
                       <div className="p-8 relative z-10">
                         <div className="text-center">
                           ...
                           <h3 className="text-2xl font-playful text-green-700 mb-6">Fastest Posture</h3>
                           
                           {smallestAvgTime ? (
                             <div className="space-y-4">
                               <div className="bg-white/70 rounded-2xl p-6 border border-green-200/50">
                                 <div className="text-3xl mb-3">{smallestAvgTime.posture}</div>
                           ...
                           {highestAvgTime ? (
                             <div className="space-y-4">
                               <div className="bg-white/70 rounded-2xl p-6 border border-red-200/50">
                                 <div className="text-3xl mb-3">{highestAvgTime.posture}</div>
                   ...
                   <div className="relative bg-gradient-to-br from-yellow-50/90 to-orange-50/90 backdrop-blur-sm rounded-3xl border-2 border-yellow-200/70 shadow-xl p-8">
                     <div className="text-center">
                       ...
                       <p className="text-lg font-comic text-orange-600 mb-6 max-w-2xl mx-auto">
                         Every practice session makes you better at facial expressions. 
                 ...
                 <div className="relative">
                   ...
                   <div className="relative bg-gradient-to-br from-white/95 to-white/85 backdrop-blur-sm rounded-3xl border-2 border-blue-200/50 shadow-2xl p-8">
                     <div className="text-center mb-8">
                       ...
                       <p className="text-lg font-comic text-gray-600">
                         Deep dive into {selectedChild.name}'s facial expression mastery!
                       </p>
                     </div>
                   </div>
                 </div>
                       ...
                       <div className="p-8 relative z-10">
                         <div className="text-center mb-6">
                           ...
                           <p className="text-sm font-comic text-blue-600">
                             How long it takes to complete each posture on average
                           </p>
                         </div>
                         ...
                         <div className="bg-white/70 rounded-2xl p-4 border border-blue-200/50">
                           <ResponsiveContainer width="100%" height={300}>
                             <BarChart data={averageTimesData}>
                               ...
                               <XAxis dataKey="posture" angle={-45} textAnchor="end" height={80} tick={{ fontSize: 10 }} />
                       ...
                       <div className="p-8 relative z-10">
                         <div className="text-center mb-6">
                           ...
                           <p className="text-sm font-comic text-purple-600">
                             Percentage of games where each posture was completed
                           </p>
                         </div>
                         ...
                         <div className="space-y-4">
                           {averageTimesData
                             ...
                             .map((item, index) => (
                               <div key={item.posture} className="group/item bg-white/70 rounded-xl border border-purple-200/50 p-4 hover:scale-105 transition-all duration-300 hover:shadow-lg">
                                 <div className="flex items-center justify-between mb-3">
                                   <div className="flex items-center gap-2">
                                     <span className="text-2xl">{item.posture.split(' ').pop()}</span>
                                     <span className="font-comic text-sm font-medium text-purple-700">
                                       {item.posture.split(' ').slice(0, -1).join(' ')}
                                     </span>
                                   </div>
                         ...
                         <div className="text-center">
                           ...
                           <div className="bg-white/70 rounded-2xl p-6 border border-orange-200/50">
                             <div className="text-5xl font-bold text-orange-600 mb-2 animate-pulse">
                               {performanceMetrics?.improvementTrend ? performanceMetrics.improvementTrend.toFixed(1) : '0'}%
                             </div>
                             <div className="text-lg font-comic text-orange-700 font-semibold mb-2">
                               Recent Performance
                             </div>
                             <div className="text-sm text-orange-600">
                               {performanceMetrics?.improvementTrend && performanceMetrics.improvementTrend > 0 
                   ...
                   <div className="group relative">
                     ...
                     <div className="relative bg-gradient-to-br from-pink-50/90 to-rose-50/90 backdrop-blur-sm rounded-3xl border-2 border-pink-200/70 shadow-xl hover:shadow-2xl transition-all duration-300 overflow-hidden">
                       ...
                       <div className="p-8 relative z-10">
                         <div className="text-center">
                           <div className="text-5xl mb-4 animate-wiggle">🥧</div>
                           <h3 className="text-2xl font-playful text-pink-700 mb-6">Posture Distribution</h3>
                           ...
                           <div className="bg-white/70 rounded-2xl p-4 border border-pink-200/50">
                             {postureDistributionData.length > 0 ? (
                               ...
                               <div className="text-center text-muted-foreground p-4">
                                 ...
                                 <div className="text-lg font-comic text-pink-600">No data available yet</div>
                                 <div className="text-sm text-pink-500">Start playing to see the distribution!</div>
                               </div>
                             )}
                           </div>
                         </div>
                   ...
                   <div className="relative bg-gradient-to-br from-indigo-50/90 to-purple-50/90 backdrop-blur-sm rounded-3xl border-2 border-indigo-200/70 shadow-xl p-8">
                     <div className="text-center">
                       ...
                       <p className="text-lg font-comic text-indigo-600 mb-6 max-w-2xl mx-auto">
                         {selectedChild.name} is showing amazing progress in facial expressions! 
                 ...
                 <div className="grid grid-cols-1 gap-6">
                   <Card className="card-playful backdrop-blur-sm bg-white/90 border-2 border-purple-200 shadow-xl">
                     ...
                     <CardContent>
                       {sessionData.length > 0 ? (
                         <div className="space-y-4">
                           {POSTURE_NAMES.map((postureName) => {
                             const averageTime = getAverageTime(postureName);
                             const lastSession = sessionData[sessionData.length - 1];
                             
                             // Get the last session's time for this specific posture
                             let lastSessionTime = null;
                             if (lastSession && lastSession.postureTimes) {
                               lastSessionTime = lastSession.postureTimes[postureName] || null;
                             }
                             
                             // Show all postures, even if not completed in last session
                             ...
                             return (
                               <div key={postureName} className="p-4 bg-gradient-to-r from-purple-50 to-blue-50 rounded-xl border border-purple-200 hover:scale-105 transition-all">
                                 <div className="flex items-center justify-between mb-3">
                                   <div className="flex items-center gap-2">
                                     <span className="text-2xl">{postureName.split(' ').pop()}</span>
                                     <span className="font-comic text-base font-medium">{postureName.split(' ').slice(0, -1).join(' ')}</span>
                                   </div>
               ...
               <TabsContent value="consistency" className="space-y-6">
                 <Card className="card-playful backdrop-blur-sm bg-white/90 border-2 border-blue-200 shadow-xl">
                   <CardHeader className="pb-3">
                     ...
                     <CardDescription className="font-comic text-base">
                       How consistent the performance is for each posture
                     </CardDescription>
                   </CardHeader>
                   <CardContent>
                     <ResponsiveContainer width="100%" height={300}>
                       <RadarChart data={consistencyData}>
                         <PolarGrid />
                         <PolarAngleAxis dataKey="posture" tick={{ fontSize: 10 }} />
               ...
               <TabsContent value="history" className="space-y-6">
                 <Card className="card-playful backdrop-blur-sm bg-white/90 border-2 border-purple-200 shadow-xl">
                   <CardHeader className="pb-3">
                     ...
                     <CardDescription className="font-comic text-base">
                       Your recent performance history with detailed posture breakdowns
                     </CardDescription>
                   </CardHeader>
                   <CardContent>
                     <div className="space-y-4">
                       {gameHistory.map((record, index) => {
                         const totalTime = POSTURE_NAMES.reduce((sum, posture) => {
                           const time = getPostureTime(record, posture);
                           return sum + (time || 0);
                         }, 0);
                         const completedPostures = POSTURE_NAMES.filter(posture => {
                           const time = getPostureTime(record, posture);
                           return time && time > 0;
                         }).length;
                         const averageTime = completedPostures > 0 ? totalTime / completedPostures : 0;
                         const completionRate = (completedPostures / POSTURE_NAMES.length) * 100;
                           ...
                           <div key={record.id} className="border rounded-lg p-4 hover:bg-gray-50 transition-colors">
                             ...
                             <div className="grid grid-cols-3 gap-2 mb-3">
                               <div className="text-center p-2 bg-blue-50 rounded">
                                 <div className="text-sm font-bold text-blue-600">{completedPostures}</div>
                             ...
                             <div className="border-t pt-3">
                               <div className="text-xs font-semibold text-gray-600 mb-2">Posture Performance:</div>
                               <div className="grid grid-cols-2 gap-2">
                                 {POSTURE_NAMES.map((posture) => {
                                   ...
                                   const emoji = posture.split(' ').pop(); // Get the emoji
                                   const name = posture.split(' ').slice(0, -1).join(' '); // Get the name without emoji
                                   
                                   return (
                                     <div key={posture} className={`flex items-center justify-between p-2 rounded text-xs ${
