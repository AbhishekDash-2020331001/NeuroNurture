...
import { Conversation, NuruService, Message as ServiceMessage } from '../../../shared/services/nuruService';
...
const Assistant: React.FC<AssistantProps> = ({ isOpen, onToggle }) => {
  ...
  useEffect(() => {
    const loadConversations = async () => {
      ...
      try {
        const response = await NuruService.getConversations('admin', admin.id);
  ...
  const loadMessages = async (conversationId: string) => {
    ...
    try {
      const response = await NuruService.getMessages('admin', admin.id, conversationId);
  ...
  const deleteChat = async (chatId: string) => {
    ...
    try {
      await NuruService.deleteConversation('admin', admin.id, chatId);
